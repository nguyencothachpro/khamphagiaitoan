...
function buildProgressiveMap(map,steps,index){
  ...
  return '<div class="guided-map" data-map="'+index+'"><div class="guided-map-head"><span>🎓</span><div><b>Học sinh tự dựng sơ đồ — '+esc(map.method_title||("Cách giải "+(index+1)))+'</b><small>'+esc(map.method_note||"Giáo viên hỏi → học sinh suy luận → sơ đồ hình thành.")+'</small></div></div><div class="guided-progress"><span class="progress-count">Bước 1/'+steps.length+'</span><div class="progress-track"><i style="width:0%"></i></div></div><div class="pm-live-map"><div class="pm-live-root hidden" data-piece="'+esc(map.root||"Bài toán")+'">'+esc(map.root||"Bài toán")+'</div><div class="pm-live-vline hidden"></div><div class="pm-live-branches hidden">'+branches+'</div><div class="pm-live-hub hidden"></div><div class="pm-live-conclusion hidden"><div data-piece="'+esc(map.conclusion_label||"")+'">'+esc(map.conclusion_label||"")+'</div><strong data-piece="'+esc(map.conclusion_value||"")+'">'+esc(map.conclusion_value||"")+'</strong></div></div><div class="guided-current">'+data+'<div class="guided-live-kicker">GV DẪN DẮT — BƯỚC <span class="current-no">1</span></div><h4 class="current-title"></h4><div class="guided-question current-question"></div><div class="student-goal current-goal"></div><textarea class="guided-answer current-answer" rows="2" placeholder="Học sinh trả lời tại đây…"></textarea><div class="guided-actions"><button class="hint-btn" type="button">💡 Gợi ý</button><button class="confirm-btn" type="button">✓ Đã suy luận → mở mảnh sơ đồ</button></div><div class="guided-hint hidden current-hint"></div><div class="guided-wrong hidden current-wrong"></div><details class="teacher-answer"><summary>👨‍🏫 Giáo viên xem đáp án</summary><div class="current-expected"></div></details><div class="unlocked-piece hidden current-unlock"></div></div><div class="guided-complete hidden"><div class="complete-badge">🎉 Học sinh đã tự dựng xong sơ đồ</div><div class="pm-final-explanation">💡 Chú giải: '+esc(map.explanation||"")+'</div><div class="complete-next">Bây giờ hỏi: <b>“Các mảnh này liên hệ với nhau thế nào để tạo thành cách giải?”</b></div></div></div>';
}
...
function initGuidedMaps(){
  document.querySelectorAll(".guided-map").forEach(map=>{
    ...
    const no=map.querySelector(".current-no"),title=map.querySelector(".current-title"),question=map.querySelector(".current-question"),goal=map.querySelector(".current-goal"),answer=map.querySelector(".current-answer"),hint=map.querySelector(".current-hint"),wrong=map.querySelector(".current-wrong"),expected=map.querySelector(".current-expected"),unlock=map.querySelector(".current-unlock"),count=map.querySelector(".progress-count"),bar=map.querySelector(".progress-track i"),complete=map.querySelector(".guided-complete");
    ...
    function renderStep(){
      ...
      answer.value="";hint.classList.add("hidden");wrong.classList.add("hidden");unlock.classList.add("hidden");answer.focus();
    }
